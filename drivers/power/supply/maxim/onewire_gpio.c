...
void Delay_us(unsigned int T) { udelay(T); }
EXPORT_SYMBOL(Delay_us);

void Delay_ns(unsigned int T) { ndelay(T); }
...
unsigned char read_byte(void) {
        ...
        for (i = 0; i < 8; i++) {
                if (read_bit())
                        value |= 0x01 << i; // reads byte in, one byte at a time
...
int onewire_gpio_get_status(void) { return onewire_gpio_detected; }
...
static int onewire_gpio_probe(struct platform_device *pdev) {
        ...
        ow_log("lvchen1\n");
        if (pdev->dev.of_node) {
                onewire_data = devm_kzalloc(
                    &pdev->dev, sizeof(struct onewire_gpio_data), GFP_KERNEL);
                if (!onewire_data) {
        ...
        if (!retval && onewire_data->ow_gpio_pinctrl) {
                retval =
                    pinctrl_select_state(onewire_data->ow_gpio_pinctrl,
        ...
        onewire_data->gpio_in_out_reg = devm_ioremap(
            &pdev->dev, (uint32_t)onewire_data->onewire_gpio_level_addr, 0x4);
        ...
        ow_log("onewire_data->gpio_cfg66_reg is %x; "
               "onewire_data->gpio_in_out_reg is %x",
