...
int dsi_panel_set_doze_backlight(struct dsi_display *display)
{
	...
	if (!dsi_display || !dsi_display->panel || !dsi_display->drm_dev) {
		pr_debug("invalid display/panel/drm_dev\n");
		return -EINVAL;
	}
...
int dsi_panel_set_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if (0 == bl_lvl){
		if(panel->fod_dimlayer_hbm_enabled){
			pr_debug("skip set backlight=0 bacase fod_dimlayer_hbm_enabled enable");
			return 0;
		}
		if (!panel->in_aod)
	...
	if (panel->fod_dimlayer_bl_block && bl_temp != 0) {
		...
		pr_debug("skip set backlight due to dim layer block\n");
		return rc;
	}

	if (panel->dc_enable && bl_temp < panel->dc_threshold && bl_temp != 0) {
		pr_debug("skip set backlight bacase dc enable %d, dim_layer_replace_dc:%d, bl %d, last_bl %d\n",
...
int dsi_panel_parse_elvss_dimming_read_configs(struct dsi_panel *panel)
{
	...
	if (!panel->elvss_dimming_offset.count) {
		pr_debug("elvss dimming offset command parsing failed\n");
		return -EINVAL;
	}
	...
	if (!elvss_dimming_cmds->read_cmd.count) {
		pr_debug("elvss dimming command parsing failed\n");
		return -EINVAL;
	}
	...
	if (rc) {
		pr_debug("failed to parse elvss dimming read length, rc=%d\n", rc);
		return -EINVAL;
	}
	...
	if (!panel->hbm_fod_on.count) {
		pr_debug("hbm fod on command parsing failed\n");
		return -EINVAL;
	}
	...
	if (!panel->hbm_fod_off.count) {
		pr_debug("hbm fod off command parsing failed\n");
		return -EINVAL;
	}
	...
	if (!panel->hbm_fod_off_doze_hbm_on.count) {
		pr_debug("hbm fod off doze hbm on command parsing failed\n");
		return -EINVAL;
	}
	...
	if (!panel->hbm_fod_off_doze_hbm_on.count) {
		pr_debug("hbm fod off doze lbm on command parsing failed\n");
		return -EINVAL;
	}
	return 0;
}
...
static int dsi_panel_parse_esd_config(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(esd_config->esd_err_irq_gpio)) {
		...
		if (rc)
			pr_debug("%s: Failed to get esd irq gpio %d (code: %d)",
...
static void panelon_fod_enable_delayed_work(struct work_struct *work)
{
	...
	if (display && panel) {
		...
		if (panel->fod_dimlayer_bl_block && panel->panel_initialized) {
			pr_debug("fod_dimlayer_bl_block delay false\n");
...
int dsi_panel_set_nolp(struct dsi_panel *panel)
{
	...
	if (!panel->fod_hbm_enabled && !panel->fod_dimlayer_hbm_enabled) {
		if (panel->fodflag) {
			pr_debug("%s fod_dimlayer_bl_block\n", __func__);
...
static int panel_disp_param_send_lock(struct dsi_panel *panel, int param)
{
...
#else
				pr_debug("HBM_FOD_OFF DSI_CMD_SET_DOZE_HBM");
				rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_DOZE_HBM);
				if (rc)
					pr_debug("[%s] failed to send DSI_CMD_SET_DOZE_HBM cmd, rc=%d\n",
						   panel->name, rc);
	...
	case DISPPARAM_FOD_BACKLIGHT:
		...
		if (fod_backlight == 0x1000) {
			...
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_DISP_DIMMINGOFF);
			if (panel->dc_enable) {
				pr_debug("FOD backlight restore dc_threshold=%d, doze_state=%d",
	...
	if (match && plockdowninfo) {
		...
		if (sscanf(lockdown_str, "0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x",
				...
				&plockdowninfo[4], &plockdowninfo[5], &plockdowninfo[6], &plockdowninfo[7])
					!= 8) {
			pr_debug("failed to parse lockdown info from cmdline !\n");
		} else {
			pr_debug("lockdown info from cmdline = 0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,"
...
ssize_t dsi_panel_lockdown_info_read(unsigned char *plockdowninfo)
{
	...
	if (g_panel->cur_mode->priv_info->cmd_sets[DSI_CMD_SET_MI_READ_LOCKDOWN_INFO].cmds) {
		...
		if (rc) {
			pr_debug("[%s][%s] failed to send cmds, rc=%d\n", __func__, g_panel->name, rc);
			rc = -EIO;
			goto done;
		}
		...
		if (rc <= 0) {
			pr_debug("[%s][%s] failed to read cmds, rc=%d\n", __func__, g_panel->name, rc);
			rc = -EIO;
			goto done;
		}

		for (i = 0; i < 8; i++) {
			pr_debug("[%s][%d]0x%02x", __func__, __LINE__, ld_read_config.rbuf[i]);
				...
				struct dsi_read_config *read_config)
{
	...
	if (count == 0) {
		pr_debug("No commands to be sent\n");
		goto exit;
	}
	if (cmds->last_command) {
		cmds->msg.flags |= MIPI_DSI_MSG_LASTCOMMAND;
