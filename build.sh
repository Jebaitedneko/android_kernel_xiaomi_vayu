...
TARGET_ARCH="arm64"
[[ -f "$KERNEL_ROOT_DIR/arch/$TARGET_ARCH/configs/vayu_defconfig" ]] && \
KERNEL_CONFIG="vayu_defconfig" \
|| KERNEL_CONFIG="vayu_user_defconfig"
BASE_DTB_NAME="sm8150-v2"
ZIP_KERNEL_STR="mochi"
ZIP_DEVICE_NAME="vayu"
...
build() {
	...
	if [[ ${1} != "" && ${1} == "dtbs" ]]; then
		run "PATH=$PATH CROSS_COMPILE=$CROSS_COMPILE CROSS_COMPILE_ARM32=$CROSS_COMPILE_ARM32 make CC=\"${NEW_CC}\" O=out ARCH=$TARGET_ARCH CONFIG_BUILD_ARM64_DT_OVERLAY=y dtbs || exit"
		if [[ $BUILD_HAS_DTBO == "1" ]]; then
			make_dtboimg
		fi
	...
	run "PATH=$PATH CROSS_COMPILE=$CROSS_COMPILE CROSS_COMPILE_ARM32=$CROSS_COMPILE_ARM32 make CONFIG_DEBUG_SECTION_MISMATCH=y $(echo -e $MAKEOPTS) O=out ARCH=$TARGET_ARCH CC=\"${NEW_CC}\" -j$(($(nproc)+8)) || exit"
	...
	if [[ $BUILD_MODULES == "1" ]]; then
		if [[ $BUILD_HAS_MODULES == "1" ]]; then
			...
			run "PATH=$PATH CROSS_COMPILE=$CROSS_COMPILE CROSS_COMPILE_ARM32=$CROSS_COMPILE_ARM32 make CONFIG_DEBUG_SECTION_MISMATCH=y $(echo -e $MAKEOPTS) O=out ARCH=$TARGET_ARCH INSTALL_MOD_PATH=\"$BUILD_MODULES_DIR\" INSTALL_MOD_STRIP=1 modules_install || exit"
		fi
	fi
